...
public class ContentReviewDAO extends MonitorDAO<ContentReview>
{
    ...
    private static final String LIST_SQL =  
      ...
      + "WHERE CR.MONITOR_ID = CM.ID AND SITE_ID=? AND (CR.CREATED_DATE >= (NOW() + INTERVAL -30 DAY) OR CR.STATUS='NEW') ORDER BY CR.CREATED_DATE";
    ...
    private static final String LIST_BY_STATUS_SQL =  
      "SELECT CR.ID, CR.CREATED_DATE, CR.UPDATED_DATE, EFFECTIVE_DATE, CR.CODE, REASON, CR.STATUS, MONITOR_ID, NOTES, \"CHANGE\", CREATED_BY "
      + "FROM CONTENT_REVIEWS CR, CONTENT_MONITORS CM "
      + "WHERE CR.MONITOR_ID = CM.ID AND SITE_ID=? AND CR.STATUS=? AND (CR.CREATED_DATE >= (NOW() + INTERVAL -30 DAY) OR CR.STATUS='NEW') ORDER BY CR.CREATED_DATE";
