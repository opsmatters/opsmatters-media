...
public class SshClient extends Client
{
        ...
        throws JSchException, SftpException
    {
        SshClient ret = SshClient.builder()
            .env(env)
            .hostname(StringUtils.getEnvProperty("om-config.ssh.%s.hostname", env))
            .port(Integer.parseInt(StringUtils.getEnvProperty("om-config.ssh.%s.port", env, "0")))
            .username(StringUtils.getEnvProperty("om-config.ssh.%s.username", env))
            .password(StringUtils.getEnvProperty("om-config.ssh.%s.password", env))
