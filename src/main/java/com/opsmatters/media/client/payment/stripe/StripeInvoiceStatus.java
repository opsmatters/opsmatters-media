...
package com.opsmatters.media.client.payment.stripe;
...
/**
 * Represents the status of a Stripe invoice.
 * 
...
public enum StripeInvoiceStatus
{
    ...
    OPEN("open", InvoiceStatus.SENT),
    PAID("paid", InvoiceStatus.PAID),
    VOID("void", InvoiceStatus.CANCELLED),
    UNCOLLECTIBLE("uncollectible", InvoiceStatus.UNPAID);
     ...
     */
    StripeInvoiceStatus(String value, InvoiceStatus status)
    {
        this.value = value;
        this.status = status;
    }
     ...
     */
    public static StripeInvoiceStatus fromValue(String value)
    {
        StripeInvoiceStatus[] types = values();
        for(StripeInvoiceStatus type : types)
        {
